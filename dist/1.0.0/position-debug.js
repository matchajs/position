define("matcha/position/1.0.0/position-debug", [ "jquery-debug" ], function(require, exports, module) {
    ...
    function getOffset(element) {
        var box = element.getBoundingClientRect(), win = window, doc = win.document, docBody = doc.body, docElem = doc.documentElement;
        // < ie8 不支持 win.pageXOffset, 则使用 docElem.scrollLeft
        return {
            left: box.left + (win.pageXOffset || docElem.scrollLeft) - (docElem.clientLeft || docBody.clientLeft || 0),
            top: box.top + (win.pageYOffset || docElem.scrollTop) - (docElem.clientTop || docBody.clientTop || 0)
        };
    }
    // 获取 offsetParent 的位置
    function getParentOffset(element) {
        ...
        // 因此这里的偏移值直接设为 0 0
        if (parent[0] === docBody && parent.css("position") === "static") {
            offset = {
                top: 0,
                left: 0
            };
    ...
    function calculateOffset(x, size) {
        ...
        if (/\d%/.test(x)) {
            x = x.replace(/([\d\.]+)%/g, function(m, n) {
                return size * (n / 100) || error("Arithmetic Errors.");
            });
        }
        // 处理类似 100%+20px 的情况
        if (/[+\-*\/]/.test(x)) {
